...
class SSHClientMini_tmp{
    ...
    private byte[] V_C = "SSH-2.0-CUSTOM".getBytes(), V_S, I_C, I_S;
    ...
    private boolean verbose=false; // Changed to true for debugging
    private ECDH kex=null;
    private java.security.SecureRandom random = null;
        
    public SSHClientMini_tmp(String host, String username, int port, String password) throws Exception {                
        kex=new ECDH();
    ...
    private void connect_stream(String host, String username, int port, String password) throws Exception{                
        ...
        try{
            ...
            socket.setTcpNoDelay(true); // IMPORTANT: Disable Nagle's algorithm
        ...
        // Send KEXINIT
        write(buf);
        debug("connect stream ->: KEXINIT sent");

        // Read server KEXINIT
        buf = read();
        debug("Received server message type: " + buf.getCommand());
        j = buf.getInt();
        ...
        write(buf);
        debug("connect stream ->: KEXDH_INIT sent");
        
        // Read KEXDH_REPLY
        buf = read();
        debug("Received message type: " + buf.getCommand());
        kex.next(buf);
        ...
        if (buf.getCommand() != SSH_MSG_NEWKEYS )
            throw new Exception("invalid protocol(newkeys): " + buf.getCommand());            
        debug("Received NEWKEYS");
    ...
    public static void main(String[] args) throws Exception {
        if (args.length < 4) {
            System.out.println("Usage: java SSHClientMini <host> <username> <port> <password>");
            System.exit(1);
        }
        new SSHClientMini_tmp(args[0], args[1], Integer.parseInt(args[2]), args[3]);
    }
}
