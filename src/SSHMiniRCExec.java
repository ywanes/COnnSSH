...
class TesteSSH {
    ...
    static void runAuto(String fonteJava, int porta) {
        ...
        // bateria de EXEC: canal 'exec' do servidor validado por um cliente OpenSSH real (ssh host cmd).
        int[] rExec = runAutoExec(fonteJava, porta);
        ok += rExec[0]; total += rExec[1];
        // resumo
    ...
    // ===== Bateria de EXEC: canal 'exec' do servidor validado por um cliente OpenSSH real =====
    // Sobe um servidor (porta+3) com login por chave e roda 'ssh -i chave host <cmd>': confere que o
    // stdout chegou, o stderr (EXTENDED_DATA) nao contaminou o stdout, e o exit-status (7) veio certo.
    // So roda se o 'ssh' nativo existir; senao, pula (sem falhar).
    static int[] runAutoExec(String fonteJava, int porta) {
        int ok = 0, total = 0;
        boolean temSsh = false;
        try { Process v = new ProcessBuilder("ssh", "-V").redirectErrorStream(true).start(); v.getInputStream().readAllBytes(); temSsh = (v.waitFor() == 0); } catch (Exception e) {}
        if (!temSsh) { System.out.println("        exec: 'ssh' nativo ausente -> pulando o check de exec"); return new int[]{ 0, 0 }; }
        int pExec = porta + 3;
        java.io.File tmp = null;
        try {
            tmp = java.nio.file.Files.createTempDirectory("sshmini_exec_").toFile();
            SshKeys keys = new SshKeys();
            String uEd = novoPar(keys, "ed25519", tmp, "exec_user");
            String hEd = novoPar(keys, "ed25519", tmp, "exec_host");
            java.io.File authkeys = new java.io.File(tmp, "authorized_keys");
            java.nio.file.Files.writeString(authkeys.toPath(), java.nio.file.Files.readString(java.nio.file.Path.of(uEd + ".pub")));
            System.out.println("exec: canal exec do servidor (validado pelo ssh nativo)  [dir temp dinamico]");
            Session.authKeysFile = authkeys.getAbsolutePath();
            Session.hostKeyFile  = hEd;
            if (!sobeServidor(pExec)) { total++; System.out.println("[FALHA] exec: servidor de teste nao subiu na " + pExec); }
            else {
                java.io.File kh = new java.io.File(tmp, "known_hosts");
                int[] r = sshExec(uEd, kh.getAbsolutePath(), pExec, "admin@localhost", "echo SSHMINIMARK; echo ERRO123 1>&2; exit 7");
                boolean c = r != null && r[0] == 1 && r[1] == 7;
                total++; if (c) ok++; System.out.println((c ? "[OK]    " : "[FALHA] ") + "exec: ssh 'host cmd' -> stdout + stderr separado + exit-status 7");
            }
        } catch (Exception e) {
            total++; System.out.println("[FALHA] exec: excecao na bateria: " + e);
        } finally {
